<div>   
    ...
    <div class="shadow sm:hidden">
        <ul class="mt-2 divide-y divide-cool-gray-200 overflow-hidden shadow sm:hidden" x-max="1">
            @forelse ($asnaf_list as $asnaf)
                <li>
                    <a href="{{ route('asnaf.maklumat', ['uuid'=>$asnaf->uuid]) }}" class="block px-4 py-4 bg-white hover:bg-cool-gray-50">
                        <div class="flex items-center space-x-4">
                            <div class="flex-1 flex space-x-2">
                                ...
                                <div class="text-cool-gray-500 text-sm">
                                    <p>{{ $asnaf->name }}</p>
                                    <p>{{ $asnaf->ic_no }}</p>
                                    <p>{{ $asnaf->dependents }} Orang</p>
                                    <p>RM {{ $asnaf->household_income }}</p>
                                </div>
                            </div>
                            <div>
            ...
            @empty
                <h1 class="text-gray-400 text-sm italic">Tiada senarai asnaf zakat untuk akaun ini.</h1>
            @endforelse
        </ul>
        <div class="bg-white px-4 py-3 border-t border-cool-gray-200">
            {{ $asnaf_list->links('pagination::tailwind') }}
        </div>
    </div>
                    ...
                    <table class="min-w-full divide-y divide-cool-gray-200">
                        <thead>
                            <tr>
                                ...
                                <th class="px-6 py-3 bg-cool-gray-50 text-center text-xs leading-4 font-medium text-cool-gray-500 uppercase tracking-wider">
                                    Jumlah Pendapatan Isi Rumah (RM)
                                </th>
                                <th class="px-6 py-3 bg-cool-gray-50 text-center text-xs leading-4 font-medium text-cool-gray-500 uppercase tracking-wider">
                                    Tarikh Daftar
                                </th>
                        ...
                        <tbody class="bg-white divide-y divide-cool-gray-200" x-max="1">
                            @forelse ($asnaf_list as $asnaf)
                                <tr class="bg-white">
                                    <td class="max-w-0 px-6 py-4 whitespace-no-wrap text-sm leading-5 text-cool-gray-900">
                                        <div class="flex">
                                            <a href="{{ route('asnaf.maklumat', ['uuid'=>$asnaf->uuid]) }}" class="group inline-flex space-x-2 truncate text-sm leading-5">
                            ...
                            @empty
                                <tr>
                                    <td colspan="5" class="text-gray-400 text-sm italic p-5">Tiada senarai asnaf zakat untuk akaun ini.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
