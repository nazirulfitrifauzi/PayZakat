<div>
    <x-general.card class="pt-4 bg-white rounded-lg">
        ...
        <div class="mt-20 sm:mt-8">
            <div class="flex items-center justify-center sm:justify-end gap-5 p-5 ">
                <div class="flex flex-row-reverse gap-3">
                    ...
                    @endif
                    @if ($currentstep == 3)
                        <a href="{{ route('ppz.refund') }}" class="flex items-center justify-center w-40 py-3 text-sm leading-none text-white transition duration-300 ease-in-out bg-teal-400 rounded cursor-pointer hover:bg-teal-500">
                            Pembayaran Baru
                        </a>
                    @endif
                </div>
    ...
    <div class="p-0 sm:p-5 mx-auto mt-2 bg-white rounded-md">
        <div class="{{ ($currentstep == 1) ? 'block' : 'hidden' }}">
            @include('pages.ppz.refund.steps.step1')
        </div>
        <div class="{{ ($currentstep == 2) ? 'block' : 'hidden' }}">
            @include('pages.ppz.refund.steps.step2')
        </div>
        <div class="{{ ($currentstep == 3) ? 'block' : 'hidden' }}">
            @include('pages.ppz.refund.steps.step3')
        </div>
    </div>
</div>
