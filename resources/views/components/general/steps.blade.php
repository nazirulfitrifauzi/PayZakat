      ...
      <div class="absolute inset-0 bg-transparent flex">
            @foreach ($steps as $step => $label)
                  <div class="w-full flex items-center justify-center">
                        <button class="relative transition duration-300 ease-in-out text-md font-medium rounded-full leading-none w-13 h-13 flex justify-center cursor-default items-center border-solid border-2 border-gray-200 z-40 {{ ($currentstep > $step) ? "bg-teal-400 text-white" : "bg-gray-200 border-2 text-teal-400" }}">
